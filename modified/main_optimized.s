	.intel_syntax noprefix
allocate:
	push	rbp
	mov	rbp, rsp
	mov	edi, 1000000
	call	malloc@PLT
	mov	QWORD PTR input_string[rip], rax
	mov	edi, 1000000
	call	malloc@PLT
	mov	QWORD PTR result_string[rip], rax
	nop
	pop	rbp
	ret
dispose:
	push	rbp
	mov	rbp, rsp
	mov	rax, QWORD PTR input_string[rip]
	mov	rdi, rax
	call	free@PLT
	mov	rax, QWORD PTR result_string[rip]
	mov	rdi, rax
	call	free@PLT
	mov	rdx, QWORD PTR input_file[rip]
	mov	rax, QWORD PTR stdin[rip]
	cmp	rdx, rax
	je	.L3
	mov	rax, QWORD PTR input_file[rip]
	test	rax, rax
	je	.L3
	mov	rax, QWORD PTR input_file[rip]
	mov	rdi, rax
	call	fclose@PLT
.L3:
	mov	rdx, QWORD PTR output_file[rip]
	mov	rax, QWORD PTR stdout[rip]
	cmp	rdx, rax
	je	.L5
	mov	rax, QWORD PTR output_file[rip]
	test	rax, rax
	je	.L5
	mov	rax, QWORD PTR output_file[rip]
	mov	rdi, rax
	call	fclose@PLT
.L5:
	nop
	pop	rbp
	ret
.LC0:
	.string	"Error. Each argument must be presented as <arg tag> <arg value>"
.LC1:
	.string	"-i"
.LC2:
	.string	"-o"
.LC3:
	.string	"-g"
.LC4:
	.string	"Error. Size of random string must be an integer from 1 to %d\n"
.LC5:
	.string	"-t"
.LC6:
	.string	"Error. Testing iteration count must be a positive integer"
.LC7:
	.string	"Unknown arg tag: %s\n"
.LC8:
	.string	"r"
.LC9:
	.string	"w"
.LC10:
	.string	"Error. I/O files can't be opened"
	.text
	.globl	main
main:
	push	rbp
	mov	rbp, rsp
	sub	rsp, 48
	mov	eax, edi    # Для проверки остатка argc сразу поместим в регистр eax
	mov	r14d, eax    # Границу цикла (argc - 1) будем хранить в r14d
	sub r14d, 1
	mov	r13, rsi    # Используем r13 вместо стека для хранения argv
	mov	QWORD PTR -16[rbp], 0
	mov	QWORD PTR -8[rbp], 0
	mov	r12d, 0    # Используем r12 вместо стека для хранения size_of_random_string
	mov	DWORD PTR -24[rbp], 0
	cdq
	shr	edx, 31
	add	eax, edx
	and	eax, 1
	sub	eax, edx
	cmp	eax, 1
	je	.L7
	lea	rax, .LC0[rip]
	mov	rdi, rax
	call	puts@PLT
	mov	eax, 1
	jmp	.L8
.L7:
	mov	r15d, 1    # Используем r15 вместо стека для хранения переменной цикла i
	jmp	.L9
.L18:
	mov	eax, r15d
	cdqe
	lea	rdx, 0[0+rax*8]
	mov	rax, r13
	add	rax, rdx
	mov	rax, QWORD PTR [rax]
	lea	rdx, .LC1[rip]
	mov	rsi, rdx
	mov	rdi, rax
	call	strcmp@PLT
	test	eax, eax
	jne	.L10
	mov	eax, r15d
	cdqe
	add	rax, 1
	lea	rdx, 0[0+rax*8]
	mov	rax, r13
	add	rax, rdx
	mov	rax, QWORD PTR [rax]
	mov	QWORD PTR -16[rbp], rax
	jmp	.L11
.L10:
	mov	eax, r15d
	cdqe
	lea	rdx, 0[0+rax*8]
	mov	rax, r13
	add	rax, rdx
	mov	rax, QWORD PTR [rax]
	lea	rdx, .LC2[rip]
	mov	rsi, rdx
	mov	rdi, rax
	call	strcmp@PLT
	test	eax, eax
	jne	.L12
	mov	eax, r15d
	cdqe
	add	rax, 1
	lea	rdx, 0[0+rax*8]
	mov	rax, r13
	add	rax, rdx
	mov	rax, QWORD PTR [rax]
	mov	QWORD PTR -8[rbp], rax
	jmp	.L11
.L12:
	mov	eax, r15d
	cdqe
	lea	rdx, 0[0+rax*8]
	mov	rax, r13
	add	rax, rdx
	mov	rax, QWORD PTR [rax]
	lea	rdx, .LC3[rip]
	mov	rsi, rdx
	mov	rdi, rax
	call	strcmp@PLT
	test	eax, eax
	jne	.L13
	mov	eax, r15d
	cdqe
	add	rax, 1
	lea	rdx, 0[0+rax*8]
	mov	rax, r13
	add	rax, rdx
	mov	rax, QWORD PTR [rax]
	mov	rdi, rax
	call	atoi@PLT
	mov	r12d, eax
	cmp	r12d, 0
	jle	.L14
	cmp	r12d, 1000000
	jle	.L29
.L14:
	mov	esi, 1000000
	lea	rax, .LC4[rip]
	mov	rdi, rax
	mov	eax, 0
	call	printf@PLT
	mov	eax, 1
	jmp	.L8
.L13:
	mov	eax, r15d
	cdqe
	lea	rdx, 0[0+rax*8]
	mov	rax, r13
	add	rax, rdx
	mov	rax, QWORD PTR [rax]
	lea	rdx, .LC5[rip]
	mov	rsi, rdx
	mov	rdi, rax
	call	strcmp@PLT
	test	eax, eax
	jne	.L16
	mov	eax, r15d
	cdqe
	add	rax, 1
	lea	rdx, 0[0+rax*8]
	mov	rax, r13
	add	rax, rdx
	mov	rax, QWORD PTR [rax]
	mov	rdi, rax
	call	atoi@PLT
	mov	DWORD PTR -24[rbp], eax
	cmp	DWORD PTR -24[rbp], 0
	jg	.L30
	lea	rax, .LC6[rip]
	mov	rdi, rax
	call	puts@PLT
	mov	eax, 1
	jmp	.L8
.L16:
	mov	eax, r15d
	cdqe
	lea	rdx, 0[0+rax*8]
	mov	rax, r13
	add	rax, rdx
	mov	rax, QWORD PTR [rax]
	mov	rsi, rax
	lea	rax, .LC7[rip]
	mov	rdi, rax
	mov	eax, 0
	call	printf@PLT
	mov	eax, 1
	jmp	.L8
.L29:
	nop
	jmp	.L11
.L30:
	nop
.L11:
	add	r15d, 2
.L9:
	cmp	r15d, r14d
	jl	.L18
	# Удалено обнуление регистра eax, т.к. никаких параметров не передается
	call	allocate
	mov r15, QWORD PTR -16[rbp];    # Используем r15 вместо стека для временного хранения input_file_path
	cmp	r15, 0
	jne	.L19
	mov	rax, QWORD PTR stdin[rip]
	mov	QWORD PTR input_file[rip], rax
	jmp	.L20
.L19:
	mov	rax, r15
	lea	rdx, .LC8[rip]
	mov	rsi, rdx
	mov	rdi, rax
	call	fopen@PLT
	mov	QWORD PTR input_file[rip], rax
.L20:
	mov r15, QWORD PTR -8[rbp];    # Используем r15 вместо стека для временного хранения output_file_path
	cmp	r15, 0
	jne	.L21
	mov	rax, QWORD PTR stdout[rip]
	mov	QWORD PTR output_file[rip], rax
	jmp	.L22
.L21:
	mov	rax, r15
	lea	rdx, .LC9[rip]
	mov	rsi, rdx
	mov	rdi, rax
	call	fopen@PLT
	mov	QWORD PTR output_file[rip], rax
.L22:
	mov	rax, QWORD PTR input_file[rip]
	test	rax, rax
	je	.L23
	mov	rax, QWORD PTR output_file[rip]
	test	rax, rax
	jne	.L24
.L23:
	lea	rax, .LC10[rip]
	mov	rdi, rax
	call	puts@PLT
	# Удалено обнуление регистра eax, т.к. никаких параметров не передается
	call	dispose
	mov	eax, 1
	jmp	.L8
.L24:
	cmp	r12d, 0
	jle	.L25
	call	generate_random@PLT
	mov	DWORD PTR is_generated[rip], 1
	jmp	.L26
.L25:
	# Удалено обнуление регистра eax, т.к. никаких параметров не передается
	call	read_input@PLT
.L26:
	cmp	DWORD PTR -24[rbp], 0
	jle	.L27
	mov	eax, DWORD PTR -24[rbp]
	mov	edi, eax
	call	test@PLT
	jmp	.L28
.L27:
	# Удалено обнуление регистра eax, т.к. никаких параметров не передается
	call	build_result_string@PLT
	# Удалено обнуление регистра eax, т.к. никаких параметров не передается
	call	write_output@PLT
.L28:
	# Удалено обнуление регистра eax, т.к. никаких параметров не передается
	call	dispose
	mov	eax, 0
.L8:
	leave
	ret
